            ...
            for asset in assets {
                ...
                if (mediaType == DKImagePickerControllerAssetType.allPhotos) {
                    ...
                    asset.fetchAVAssetWithCompleteBlock({ (AVAsset, info) in
                        ...
                        // Currently, the above 4 lines that use AVAsset!.URL
                        // aren't working, meaning it's currently not possible
                        // to export videos from the device using the multi-picker
                        // To alert users about this, an alert message will be
                        // shown, until the picker works as expected.
                        // The error message being generated by Xcode is:
                        // - Value of type 'AVAsset' has no member 'URL'
                        let exportVideoUnavailableController = UIAlertController(title: "Unable to Export Video", message: "Unfortunatley there is currently a problem exporting multiple video files. Please use the basic uploader, which can be selected in iOS settings", preferredStyle: UIAlertControllerStyle.alert)
                        exportVideoUnavailableController.addAction(UIAlertAction(title: "OK", style: UIAlertActionStyle.default, handler: nil))
                        self.present(exportVideoUnavailableController, animated: true, completion: nil)
                    })
                }
            }
